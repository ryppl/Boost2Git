    ...
    if (isIncludeRule)
      ...
      if (state == ReadingRepository)
        ...
        if (matchRepoLine.exactMatch(line))
          ...
          if (!subst.isValid())
            {
            qFatal("Malformed substitution in rules file: line %d: %s", lineNumber, qPrintable(origLine));
            }
      ...
      if (isRepositoryRule)
        ...
        if (!match.rx.isValid())
          ...
          qPrintable(matchLine.cap(1)), qPrintable(filename), lineNumber, qPrintable(match.rx.errorString()));
